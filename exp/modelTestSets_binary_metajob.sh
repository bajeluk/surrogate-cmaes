...
#   EXPLOG         -- string with the logging experiment name
...
MATLAB_BINARY_CALL="exp/metacentrum_modeltestsets"

DATASET="$EXPPATH_SHORT/$EXPLOG/${EXPLOG}_results_${FUNC}_${DIM}D_*.mat"
# if [ -z "$DATASET" ]; then
#   DATASET="$EXPPATH_SHORT/$EXPLOG/dataset/DTS_005.mat"
#   echo "There was no dataset, setting the dataset to default: $DATASET"
# elif ! grep -q '/.*\.mat$' <<< "$DATASET"; then
#   DATASET="$EXPPATH_SHORT/$EXPLOG/dataset/${DATASET}.mat"
...
cd "$EXPPATH_SHORT/../.."
mkdir "$SCRATCHDIR/$EXPLOG"
# copy logging results
cp $DATASET "$SCRATCHDIR/$EXPLOG"
# copy scmaes_params.mat
cp "$EXPPATH_SHORT/$EXPLOG/scmaes_params.mat" "$SCRATCHDIR/$EXPLOG"
...
#
echo '##############'
echo Will be called: $MATLAB_BINARY_CALL \"$EXPID\" \"$EXPPATH_SHORT\" \"$FUNC\" \"$DIM\" \"$INST\" \"$EXPLOG\" \"$DATASET\"
echo '##############'

$MATLAB_BINARY_CALL "$EXPID" "$EXPPATH_SHORT" "$FUNC" "$DIM" "$INST" "$EXPLOG" "$DATASET"
#
########################
