#!/bin/bash
#
# Job-submitting experiment for 100 RF model settings testing selected using lhs design from 400 original.
# It is expected to be located in exp/experiments/

export EXPID='exp_DTSmodels_meta_03_rf_validation_full_lhs'
export DATASET="exp/experiments/dataset/DTS_meta_005_validation.mat"

# Enable this option for using Matlab MCR compilated binaries:
export useMCR=1

# !!! THIS HAS TO BE HERE !!!
#     vvvvvvvvvvvvvvvvvvv
CWD=$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )
. $CWD/../bash_settings.sh
. $CWD/../metacentrum_testmodels_common.sh
#     ^^^^^^^^^^^^^^^^^^^
# !!! THIS HAS TO BE HERE !!!

# job submittion can be done via one of the two bash functions:
#
# (a) subtask [JOBNAME_SUFFIX]
#
#     submits a job with current-set $ID, $DIM, $FUNC, $INST and $OPTS
#
#
# (b) submit_sequence LOW_IDX STEP HIGH_IDX
#
#     call subtask() with 'modelOptionsIndices' set to a small
#     numbers of model-settings;
#     it submits a job for every $2 consecutive modelOptions' indices
#
#     e.g.: submit 11 4 22
#     submits jobs for the following modelOption indices:
#       - 11:14
#       - 15:18
#       - 19:22
#
#     Important: the HIGH_IDX should be the equal to (k*LOW_IDX) - 1
#                for some k


# critical characters has to be replaced in $OPTS:
# '|' with ',' and "%" with "'"
MODEL_IDS="2 7 10 12 13 22 24 30 36 39 42 46 47 52 55 57 68 76 78 79 87 89 91 98 107 114 116 119 124 129 130 131 134 136 140 149 157 158 159 160 171 181 185 187 190 192 193 194 198 201 202 203 205 206 211 222 228 234 239 240 245 246 247 249 257 259 262 264 265 271 279 280 286 292 300 301 304 310 316 317 321 322 323 324 325 327 332 337 339 343 349 373 376 377 379 382 385 388 396 397"

INST="[11 12 13 14 15]"
IDS="[1]" # 7 is ADD kernel
#FAILED_LINE=`exp/metacentrum_list.sh -e $EXPID {1..960} | grep -A1 "Errored out" | tail -n1`
#read -a FAILED <<< $FAILED_LINE
#MISSING=("f1_10D" "f1_20D" "f2_20D" "f3_20D" "f4_20D" "f5_20D" "f6_10D" "f6_20D" "f7_10D" "f7_20D" "f8_10D" "f8_20D" "f9_10D" "f9_20D")
#if [[ ${#FAILED[@]} -eq 0 ]]; then
#  echo "No failed ids found."
#  exit
#fi
#
#echo -n "Submit ${#FAILED[@]} failed ids (${FAILED[@]})? [y/N] "
#read ANS

#if [[ $ANS = [yY] ]]; then
#  echo "Submitting..."
#  sleep 1
#else
#  exit
#fi

ID=1

for DIM in 2 3 5 10 20; do
  case $DIM in
    [235])
      QUEUE="2:00:00"
      ;;
    10)
      QUEUE="4:00:00"
      ;;
    *)
      QUEUE="24:00:00"
      ;;
  esac

  for FUNC in `seq 1 24`; do
    for MODEL_ID in $MODEL_IDS; do
      OPTS="struct(%modelOptionsIndices%|$MODEL_ID)"

      #echo "${FAILED[@]}" | grep "\b$ID\b" > /dev/null
      #echo "${MISSING[@]}" | grep "\bf${FUNC}_${DIM}D\b" > /dev/null
      if [[ $? -eq 0 ]]; then
        subtask
      else
        ID=$((ID+1))
      fi
    done
  done
done
