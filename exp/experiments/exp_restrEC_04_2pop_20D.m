exp_id = 'exp_restrEC_04_2pop_20D';
exp_description = 'Surrogate CMA-ES model using restricted individual EC with sd2 criterion and GPs with double-population, (a) f-value normalization (b) non-normalized; GPs with improved performance; 20D';
...
bbParamDef(4).values = {[43:50]}; % 31:40]};   % default is [1:5, 31:40]
...
sgParamDef(13).values = {0.05};

% CMA-ES parameters
cmParamDef(1).name   = 'PopSize';
cmParamDef(1).values = {'(8 + floor(6*log(N)))'}; %, '(8 + floor(6*log(N)))'};
