...
#
# support script for model test set creation submitter
#
# Usage: to be called in experiment submitter via
#
# . ../metacentrum_modeltestsets_common.sh
#
# see also:
...
METACENTRUM_MODEL_BINARY=$EXPPATH_SHORT/../metacentrum_modeltestsets
...
if [ "$HELP" = 1 ]; then
  ...
  echo ""
  echo "usage: $ exp/exp_MODELTESTSETS_EXPERIMENT.sh"
  echo ""
  echo "options:"
  ...
  echo "  exp/metacentrum_modeltestsets_common.sh "
  exit 0
fi

if [ "$useMCR" = 1 ]  &&  ! make -q sets; then
...
subtask() {
  ...
  if [ "$useMCR" = 1 ]; then
    echo "MCR binary submit: ID=$ID : DIM=$DIM : FUNC=$FUNC : INST=$INST : EXPLOG=$EXPLOG : DATASET=$DATASET"
    if [ "$DRY_RUN" = 0 ]; then
      qsub -N "${EXPID}__${ID}${JOBNAME_SUFFIX}" -l "walltime=$QUEUE" -v FUNC,DIM,INST,EXPID,EXPLOG,EXPPATH_SHORT,DATASET $EXPPATH_SHORT/../modelTestSets_binary_metajob.sh
    else
      echo qsub -N "${EXPID}__${ID}${JOBNAME_SUFFIX}" -l "walltime=$QUEUE" -v FUNC,DIM,INST,EXPID,EXPLOG,EXPPATH_SHORT,DATASET $EXPPATH_SHORT/../modelTestSets_binary_metajob.sh
    fi

  else
    echo "ID=$ID : DIM=$DIM : FUNC=$FUNC : INST=$INST : MATLAB_FCN=$MATLAB_FCN : EXPLOG=$EXPLOG : DATASET=$DATASET"
    if [ "$DRY_RUN" = 0 ]; then
      qsub -N "${EXPID}__${ID}${JOBNAME_SUFFIX}" -l "walltime=$QUEUE" -v FUNC,DIM,INST,MATLAB_FCN,EXPID,EXPLOG,EXPPATH_SHORT,DATASET $EXPPATH_SHORT/../modelTestSets_metajob.sh && echo "submitted ok."
    else
      echo qsub -N "${EXPID}__${ID}${JOBNAME_SUFFIX}" -l "walltime=$QUEUE" -v FUNC,DIM,INST,MATLAB_FCN,EXPID,EXPLOG,EXPPATH_SHORT,DATASET $EXPPATH_SHORT/../modelTestSets_metajob.sh && echo "submitted ok."
    fi
  fi
  ID=$((ID+1))
}
