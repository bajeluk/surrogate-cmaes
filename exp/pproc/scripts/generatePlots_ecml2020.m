...
newFeatNames = {'$\varphi_\mathrm{med}(\mathcal{A})$', ...
                '$\varphi_{\varepsilon_s}(\mathcal{T}\cup\mathcal{P})$'};
nfn = 0;
...
%                 'XLabelName', sprintf('%d-quantile of observations', nQuant));
    ...
    h(end+1) = mftPropPlot(shiftdim(obs_vars(ft, :, :), 1), med_obs_nans, ...
                ...
                'InfLabel', '\infty', ...
                'NaNLabel', '  \bullet', ...
                'MftName', newFeatNames{nfn}, ...
                'StatLabel', 'Feature variance', ...
                'ValueName', {'0.05', '0.5', '0.95'}, ...
                'ValueStyle', {'-.', '-', '-.'}, ...
...
for i = 1:4
  hp = get(h(i), 'InnerPosition');
  set(h(i), 'InnerPosition', [hp(1:3), hp(4)/2])
end
