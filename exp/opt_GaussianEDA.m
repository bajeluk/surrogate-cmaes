function [x, y_evals, stopflag, varargout] = opt_GaussianEDA(FUN, dim, ftarget, maxfunevals, id, varargin)
...
% varargin/2 can be 'cmOptions' -- options for the CEDA algorithm
  ...
  cmOptions = struct( ...
    ...
    'UBounds',  5, ...
    'PopSize', '200*dim', ...
    'GaussianModelType', 'GaussianUnivModel', ...
    'SelectionRatio', 0.1);
  ...
  switch lower(cmOptions.GaussianModelType)
  case 'gaussianunivmodel'
    edaparams{1} = {'learning_method','LearnGaussianUnivModel',{}};
    edaparams{2} = {'sampling_method','SampleGaussianUnivModel',{myeval(cmOptions.PopSize),1}};
  case 'gaussianfullmodel'
    edaparams{1} = {'learning_method','LearnGaussianFullModel',{}};
    edaparams{2} = {'sampling_method','SampleGaussianFullModel',{myeval(cmOptions.PopSize),2}};
  case 'gaussiannetwork'
    BN_params = {'k2',10,0.05,'pearson','bic','no'};
    edaparams{1} = {'learning_method','LearnGaussianNetwork',BN_params};
    edaparams{2} = {'sampling_method','SampleBN',{myeval(cmOptions.PopSize),1}};
  end
