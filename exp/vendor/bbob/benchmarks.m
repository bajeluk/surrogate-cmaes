...
% benchmark chemical function using fitness_PRODMECH
% created according to template
...
%   x = linearTF * x;    % rotate/scale
  ...
  if DIM > 3
    x = (x(1:4,:) + 5)/10.*repmat(ub-lb,1,POPSI)+repmat(lb,1,POPSI);
  else
    x = ([x(1:2,:);0;0] + 5)/10.*repmat(ub-lb,1,POPSI)+repmat(lb,1,POPSI);
  end
