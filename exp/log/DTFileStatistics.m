  ...
  methods
    ...
    function notify(obj, ec, varargin)
      ...
      if (ec.cmaesState.countiter == 1 && ec.counteval <= (ec.cmaesState.lambda+1))
        ...
        fprintf(fid, '#  iter  totEvals  orEvalsPop  Dopt  rmseReeval  rnkReeval  rnk2Models rnkValid  modelTypeUsed  nTrainData  nDataInRange  sigmaPow2  adaptRankDiff  usedOrigRatio  nextOrigRatio  adaptGain  adaptNewRatio  diagD_max  diagD_min  diagD_ratio  %s\n', sprintf('diagD_%02d ', 1:length(ec.cmaesState.diagD)));
      end
      ...
      %            iter  tot orEvl Dopt rmseR  rnkR  rnk2 rnkV mType nDa nDiR sigma^2 rnkDf oRatT oRatN aGain aNeRat mxDi miDi Drio D_1,...,D_n
      fprintf(fid, '%4d  %5d  %2d  %.4e  %.2e  %.2f  %.2f  %.2f  %d  %2d  %2d  %.2e  %.2f  %.2f  %.2f  %.2f  %.2f  %.2e  %.2e  %.2e  %s\n', ...
