function [fitness_raw, arx, arxvalid, arz, archive, counteval, xTrain, yTrain] = presample(minTrainSize, surrogateOpts, cmaesState, archive, xTrain, yTrain, varar)
  ...
  if (missingTrainSize > maxPresampleSize)
    ...
    [fitness_raw, arx, arxvalid, arz, counteval] = sampleCmaes(xmean, sigma, lambda, BD, diagD, fitfun_handle, surrogateOpts.sampleOpts, varar); % varar should be varargin{:});
  ...
  if (missingTrainSize > 0)
    ...
    [fitness_raw, arx, arxvalid, arz, counteval] = ...
        sampleCmaesOnlyFitness(xPreSample, xPreSample, zPreSample, xmean, expandedSigma, missingTrainSize, BD, diagD, fitfun_handle, surrogateOpts.sampleOpts, varar); % varar should be varargin{:});
    archive = archive.save(arxvalid', fitness_raw', countiter);
